...
// ── IndexedDB backup (survives PWA restarts on iOS when localStorage gets wiped) ──
let __db=null;
function __openDB(){return new Promise(r=>{try{const q=indexedDB.open(‘CasinoDB’,1);q.onupgradeneeded=e=>e.target.result.createObjectStore(‘kv’);q.onsuccess=e=>{__db=e.target.result;r(__db);};q.onerror=()=>r(null);}catch(e){r(null);}});}
function __dbGet(k){return __openDB().then(db=>{if(!db)return null;return new Promise(r=>{try{const q=db.transaction(‘kv’,‘readonly’).objectStore(‘kv’).get(k);q.onsuccess=()=>r(q.result);q.onerror=()=>r(null);}catch(e){r(null);}});});}
function __dbSet(k,v){return __openDB().then(db=>{if(!db)return;try{db.transaction(‘kv’,‘readwrite’).objectStore(‘kv’).put(v,k);}catch(e){}});}
...
// On startup: if localStorage is empty but IndexedDB has data, restore it and re-render.
// This is what makes data persist across PWA reopens.
function __restoreFromDB(){
if(localStorage.getItem(‘ccc_v5’))return; // localStorage still has data, nothing to do
__dbGet(‘ccc_v5’).then(raw=>{
if(!raw)return;
try{localStorage.setItem(‘ccc_v5’,raw);}catch(e){}
try{
...
if(typeof renderShiftInfo===‘function’)renderShiftInfo();
if(typeof renderHomeLog===‘function’)renderHomeLog();
if(typeof renderExchangeList===‘function’)renderExchangeList();
if(typeof renderCashpointList===‘function’)renderCashpointList();
if(typeof renderAddList===‘function’)renderAddList();
if(typeof renderKFLog===‘function’)renderKFLog();
if(typeof renderShopItems===‘function’)renderShopItems();
if(typeof renderShopLog===‘function’)renderShopLog();
if(typeof recalc===‘function’)recalc();
if(typeof updateEstCalc===‘function’)updateEstCalc();
if(typeof updateHomeEst===‘function’)updateHomeEst();
}catch(e){console.warn(‘restore err’,e);}
});
// Also restore checklist/coffee/checks
__dbGet(‘ccc_checks’).then(raw=>{if(raw)try{localStorage.setItem(‘ccc_checks’,raw);}catch(e){}});
__dbGet(‘ccc_checklist’).then(raw=>{if(raw){try{localStorage.setItem(‘ccc_checklist’,raw);}catch(e){}if(typeof renderChecklist===‘function’)renderChecklist();}});
__dbGet(‘ccc_coffee’).then(raw=>{
if(!raw)return;
try{localStorage.setItem(‘ccc_coffee’,raw);}catch(e){}
try{const d=JSON.parse(raw);if(d.end&&Date.now()<d.end&&typeof loadCoffeeTimer===‘function’)loadCoffeeTimer();}catch(e){}
});
}
...
div.innerHTML=`<div class="log-ico" style="background:var(--green-dim)">🔑</div> <div class="log-body"><div class="log-title">Key Fillup — Machine ${d.machine}</div><div class="log-meta">${d.coins} playcoins · ${fmt(d.coins*20)}</div></div> <div class="log-right"><div class="log-amt">${fmt(d.coins*20)}</div><div class="log-time">${d.date}</div></div> <button class="log-del" onclick="delFillup(${entry.i})">✕</button>`;
...
div.innerHTML=`<div class="log-ico" style="background:var(--purp-dim)">🎲</div> <div class="log-body"><div class="log-title">Cashpoint<span class="cp-badge">${amtStr}</span></div><div class="log-meta">${isW?'Withdrawal':'Deposit'} · ${fl}</div></div> <div class="log-right"><div class="log-amt" style="color:${isW?'var(--blue)':'var(--purple)'}">${amtStr}</div><div class="log-time">${d.date}</div></div> <button class="log-del" onclick="delCashpoint(${entry.i})">✕</button>`;
} else {
const icon=d.type===‘playcoin’?‘🪙’:‘💵’,title=d.type===‘playcoin’?‘Playcoins Added’:‘Cash Added’;
div.innerHTML=`<div class="log-ico" style="background:var(--teal-dim)">${icon}</div> <div class="log-body"><div class="log-title">${title}</div><div class="log-meta">Replenishment</div></div> <div class="log-right"><div class="log-amt">${fmt(d.amount)}</div><div class="log-time">${d.date}</div></div> <button class="log-del" onclick="delAddition(${entry.i})">✕</button>`;
}
el.appendChild(div);
});
}
...
Object.keys(*checkStates).forEach(k=>{if(k.startsWith(i+’*’))delete _checkStates[k];});
_saveChecks();
...
d.innerHTML=`<div class="log-ico" style="background:var(--blue-dim)">🔄</div> <div class="log-body"><div class="log-title">Exchange</div><div class="log-meta">${CAT_LABELS[e.from]} → ${CAT_LABELS[e.to]}</div></div> <div class="log-right"><div class="log-amt">${fmt(e.amount)}</div><div class="log-time">${e.date}</div></div> <button class="log-del" onclick="delExchange(${i})">✕</button>`;
el.appendChild(d);
});
}
...
d.innerHTML=`<div class="log-ico" style="background:var(--purp-dim)">🎲</div> <div class="log-body"><div class="log-title">Cashpoint</div><div class="log-meta">${meta}</div></div> <div class="log-right"><div class="log-amt" style="color:${amtColor}">${amtStr}</div><div class="log-time">${c.date}</div></div> <button class="log-del" onclick="delCashpoint(${i})">✕</button>`;
el.appendChild(d);
});
}
...
d.innerHTML=`<div class="log-ico" style="background:var(--teal-dim)">${icon}</div> <div class="log-body"><div class="log-title">${title}</div><div class="log-meta">Replenishment</div></div> <div class="log-right"><div class="log-amt">${fmt(a.amount)}</div><div class="log-time">${a.date}</div></div> <button class="log-del" onclick="delAddition(${i})">✕</button>`;
el.appendChild(d);
});
}
...
function _saveChecks(){const s=JSON.stringify(_checkStates);try{localStorage.setItem(‘ccc_checks’,s);}catch(e){}__dbSet(‘ccc_checks’,s);}
...
return `<label style="display:flex;align-items:center;gap:10px;padding:7px 0;border-top:1px solid var(--border);cursor:pointer;"> <input type="checkbox" data-key="${key}" ${checked} style="width:16px;height:16px;accent-color:var(--green);flex-shrink:0;cursor:pointer;" onchange="_checkStates[this.dataset.key]=this.checked;_saveChecks()"> <span style="flex:1;font-size:.77rem;color:var(--text)">Fill <b>${s.fill}</b> playcoins</span> <span style="font-size:.73rem;color:var(--sub);font-family:'JetBrains Mono',monospace">${(s.fill*20).toLocaleString('no-NO')} kr</span> </label>`;}).join(’’);
...
d.innerHTML=`<div class="mlog-top"> <div class="mlog-title">🔑 Machine ${f.machine}</div> <div style="display:flex;align-items:center;gap:6px"><span class="mlog-badge">${typeMap[f.type]||f.type}</span><button class="mlog-del" onclick="delFillup(${i})">✕</button></div> </div> <div class="mlog-detail"><span style="color:var(--text);font-weight:600">${f.coins} playcoins · ${fmt(f.coins*20)}</span>${inputStr}${stepsHtml}<div style="margin-top:6px;color:var(--muted);font-size:.65rem">${f.date}</div></div>`;
el.appendChild(d);
});
}
...
div.innerHTML=` <div class="shop-item-top"> <div class="shop-item-icon">${p.icon}</div> <div class="shop-item-info"> <div class="shop-item-name">${p.name}</div> <div class="shop-item-price">${p.price} kr each</div> </div> <div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px"> <div class="shop-start-row"> <span style="font-size:.66rem;color:var(--sub)">Start:</span> <input class="shop-start-input" type="number" inputmode="numeric" min="0" placeholder="0" value="${start||''}" oninput="setShopStart('${p.id}',this.value)"> </div> <div class="shop-item-controls"> <button class="shop-counter-btn minus" onclick="shopSell('${p.id}',-1)">−</button> <div class="shop-count-display" id="shop-sold-${p.id}">${sold}</div> <button class="shop-counter-btn plus" onclick="shopSell('${p.id}',1)">+</button> </div> </div> </div> <div class="shop-item-row"><span class="sl">Sold</span><span class="sv">${sold} pcs</span></div> <div class="shop-item-row"><span class="sl">End count</span><span class="sv" id="shop-end-${p.id}" style="color:${end<0?'var(--red)':'var(--text)'}">${end}</span></div> <div class="shop-item-row"><span class="sl">Revenue</span><span class="sv" style="color:var(--green)">${revenue>0?revenue.toLocaleString('no-NO')+' kr':'—'}</span></div>`;
el.appendChild(div);
});
renderShopSummary();
updateHomeEst();
...
html+=`<div style="padding:7px 0;border-top:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;gap:8px"> <div> <div style="font-size:.82rem;color:var(--sub)">${p.icon} ${p.name}</div> <div style="font-size:.68rem;margin-top:2px;color:var(--muted)">Sold: <b style="color:var(--text)">${sold}</b> &nbsp;·&nbsp; End: <b style="color:${endColor}">${end}</b></div> </div> <div style="text-align:right;flex-shrink:0"> <div style="font-family:'JetBrains Mono',monospace;font-weight:600;font-size:.84rem;color:var(--text)">${rev.toLocaleString('no-NO')} kr</div> <div style="font-size:.67rem;color:var(--sub);margin-top:1px">${breakdown.replace(/<[^>]+>/g,'').replace(/[()]/g,'').trim()}</div> </div> </div>`;
});
// totals
...
const d=document.createElement(‘div’);d.className=‘log-item’;
d.innerHTML=`<div class="log-ico" style="background:var(--green-dim)">${p.icon}</div> <div class="log-body"><div class="log-title">${entry.name}</div><div class="log-meta">${entry.date}</div></div> <div class="log-right"><div class="log-amt" style="color:var(--green)">+${entry.price} kr</div></div>`;
el.appendChild(d);
});
}
...
function saveChecklistState(state){const s=JSON.stringify(state);try{localStorage.setItem(‘ccc_checklist’,s);}catch(e){}__dbSet(‘ccc_checklist’,s);}
...
{try{localStorage.removeItem(‘ccc_coffee’);}catch(e){}__openDB().then(db=>{if(db)try{db.transaction(‘kv’,‘readwrite’).objectStore(‘kv’).delete(‘ccc_coffee’);}catch(e){}});}
}
}
updateCoffeeDisplay();
}
...
{const s=JSON.stringify({end:_coffeeEnd});try{localStorage.setItem(‘ccc_coffee’,s);}catch(e){}__dbSet(‘ccc_coffee’,s);}
startCoffeeInterval();
...
const kfLbl=document.getElementById(‘kf-input-label’);if(kfLbl)kfLbl.textContent={kr:‘Win amount (kr)’,‘1cr’:‘Credits on display’,‘05cr’:‘Credits on display’}[D.kfType]||‘Win amount (kr)’;
}
selExFrom(‘cash’);
