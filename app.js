...
if(!D.shop)D.shop={starts:{},sold:{},log:[]};
// Backup to sessionStorage every load
sessionStorage.setItem(‘ccc_backup’,raw);
} else {
// Try sessionStorage backup (survives some iOS wipes)
const backup=sessionStorage.getItem(‘ccc_backup’);
if(backup){
const l=JSON.parse(backup);
D={…D,…l};
if(!D.inputs)D.inputs={home:{},shift:{},machines:{}};
if(!D.exchanges)D.exchanges=[];
if(!D.cashpoints)D.cashpoints=[];
...
localStorage.setItem(‘ccc_v5’,backup);
}
}
...
localStorage.setItem(‘ccc_v5’,json);
sessionStorage.setItem(‘ccc_backup’,json);
