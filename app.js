...
function __dbDel(k){return __openDB().then(db=>{if(!db)return;try{db.transaction(‘kv’,‘readwrite’).objectStore(‘kv’).delete(k);}catch(e){}});}
...
if(localStorage.getItem(‘ccc_v5’))return;
...
{try{localStorage.removeItem(‘ccc_coffee’);}catch(e){}__dbDel(‘ccc_coffee’);}
}
}
updateCoffeeDisplay();
}
...
loadCoffeeTimer();
checkBetbooksAlert();
// Pull from IndexedDB backup if localStorage was wiped by iOS
__restoreFromDB();