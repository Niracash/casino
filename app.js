...
// ── Persistent storage (IndexedDB primary, localStorage mirror) ──
const _IDB_NAME=‘ccc_db’, _IDB_STORE=‘kv’;
...
try{
const tx=db.transaction(_IDB_STORE,‘readonly’).objectStore(_IDB_STORE).get(k);
...
return _idb().then(db=>new Promise(res=>{
try{
const tx=db.transaction(_IDB_STORE,‘readwrite’).objectStore(_IDB_STORE).put(v,k);
...
let raw=null;
try{raw=await _idbGet(‘ccc_v5’);}catch(e){}
if(!raw){raw=localStorage.getItem(‘ccc_v5’);} // migrate from old storage
if(raw){
try{
...
}catch(e){console.warn(‘loadState parse failed’,e);}
}
// Ask browser to keep our storage around (helps with iOS PWA eviction)
...
let _saveTimer=null;
function saveState(){
// Debounce + write to BOTH backends for redundancy
...
// ── Startup: load persisted state, restore inputs, then render everything ──
(async function init(){
...
const lbl=document.getElementById(‘kf-input-label’);
if(lbl)lbl.textContent={kr:‘Win amount (kr)’,‘1cr’:‘Credits on display’,‘05cr’:‘Credits on display’}[D.kfType]||‘Win amount (kr)’;
}

// Restore any in-progress input values the user had typed
...
// Save before the page is hidden / closed — flushes any pending debounced write
window.addEventListener(‘pagehide’,()=>{
try{
const s=JSON.stringify(D);
localStorage.setItem(‘ccc_v5’,s);
_idbSet(‘ccc_v5’,s);
}catch(e){}
});
